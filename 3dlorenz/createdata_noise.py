...
sim_param = prm.system(sigma = 10.0, rho = 28.0, beta = (8.0 / 3.0), gvec = np.full(prm.dim, noise_mapping[parvalue]))

# create paths
numpaths = 10
ic = np.random.uniform(low=-1.0, high=1.0, size=(numpaths, prm.dim))
it = np.zeros((numpaths))
euler_param = prm.euler_maruyama(numsteps=100000, savesteps=1000, ft=50., ic=ic, it=it, numpaths=numpaths)
