...
for i in range(1, 11):
    ...
    for j in range(10):
        ...
        for ax, title, y in zip(axes.flat, titles, y_vals):
            ...
            ax.plot(t[j, :], y, label='initial condition '+str(ic))
            ax.set_title(title)
            ax.grid(True)

    plt.legend(bbox_to_anchor = (1.05, 1), loc = 2, borderaxespad = 0.)
    plt.suptitle('Observed data used for random time point experiments in 6D, number of time steps = ' + str(i*10+1))
...
for i in range(parval):
    f_est = f(np.array(meta_error_list[i][1].ordinary), x_est)
    ax.plot(f_est[1, :], f_est[3, :], f_est[5, :], label='time points = '+str(meta_error_list[i][0][1]))
