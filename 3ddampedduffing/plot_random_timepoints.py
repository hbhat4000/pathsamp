...
for i in range(1, 11):
    ...
    plt.suptitle('Observed data used for random time point experiments in 3Dd, number of time steps = ' + str(i*10+1))
...
plt.legend(bbox_to_anchor = (1.05, 1), loc = 2, borderaxespad = 0.)
plt.suptitle('Comparison of true drift function vs estimated drift functions in 3Dd')
...
for i in range(parval):
    f_est = f(np.array(meta_error_list[i][1].ordinary), x_est)
    ax.plot(f_est[0, :], f_est[1, :], f_est[2, :], label='time points = '+str(meta_error_list[i][0][1]))

ax.set_xlim([-2., 2.])
ax.set_ylim([-2., 2.])
ax.set_zlim([-2., 2.])
plt.legend(bbox_to_anchor = (1.05, 1), loc= 2, borderaxespad = 0.)
plt.suptitle('3D comparison of true drift function vs estimated drift functions in 3Dd')
plt.savefig('./random_timepoints/plots/3D_drift.pdf', format = 'pdf', bbox_inches='tight')
plt.close()
