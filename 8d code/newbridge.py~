                            ...
                            for k in range(0, d + 1):
                                for j in range(0, d + 1):
                                    for i in range(0, d + 1):
                                        if (i + j + k + l + m + n + r + s == d):
                                            # print("d", d, "i", i, "j", j, "k", k, "l", l, "m", m, "n", n, "index", index)
                                            y[:, index] = np.power(x[:, 0], i) * np.power(x[:, 1], j) * np.power(x[:, 2], k) * np.power(x[:, 3], l) * np.power(x[:, 4], m) * np.power(x[:, 5], n) * np.power(x[:, 6], r) * np.power(x[:, 7], s)
                                            index += 1
                            ...
                            for k in range(0, d + 1):
                                for j in range(0, d + 1):
                                    for i in range(0, d + 1):
                                        if (i + j + k + l + m + n + r + s == d):
                                            # print("d", d, "i", i, "j", j, "k", k, "l", l, "m", m, "n", n, "index", index)
                                            y[:, index] = H(i, x[:, 0]) * H(j, x[:, 1]) * H(k, x[:, 2]) * H(l, x[:, 3]) * H(m, x[:, 4]) * H(n, x[:, 5]) * H(r, x[:, 6]) * H(s, x[:, 7])
                                            index += 1
                            ...
                            for k in range(0, d + 1):
                                for j in range(0, d + 1):
                                    for i in range(0, d + 1):
                                        if (i + j + k + l + m + n + r + s == d):
                                            if (i >= 2):
                                                new_index_set = (i - 2, j, k, l, m, n, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i - 2, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            if (j >= 2):
                                                new_index_set = (i, j - 2, k, l, m, n, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j - 2, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            if (k >= 2):
                                                new_index_set = (i, j, k - 2, l, m, n, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k - 2, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            if (l >= 2):
                                                new_index_set = (i, j, k, l - 2, m, n, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l - 2, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            if (m >= 2):
                                                new_index_set = (i, j, k, l, m - 2, n, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m - 2, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            if (n >= 2):
                                                new_index_set = (i, j, k, l, m, n - 2, r, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n - 2, n] * mat[r, r] * mat[s, s]
                                            if (r >= 2):
                                                new_index_set = (i, j, k, l, m, n, r - 2, s)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r - 2, r] * mat[s, s]
                                            if (s >= 2):
                                                new_index_set = (i, j, k, l, m, n, r, s - 2)
                                                new_index = index_map[new_index_set]
                                                transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s - 2, s]

                                            transformation[index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n] * mat[r, r] * mat[s, s]
                                            index += 1
