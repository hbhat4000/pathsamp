                        ...
                        for j in range(0, d + 1):
                            for i in range(0, d + 1):
                                if (i + j + k + l + m + n == d):
                                    ...
                                    y[:, index] = np.power(x[:, 0], i) * np.power(x[:, 1], j) * np.power(x[:, 2], k) * np.power(x[:, 3], l) * np.power(x[:, 4], m) * np.power(x[:, 5], n)
                                    index += 1
                    ...
                    for k in range(0, d + 1):
                        for j in range(0, d + 1):
                            for i in range(0, d + 1):
                                if (i + j + k + l + m + n == d):
                                    if (i >= 2):
                                        new_index_set = (i - 2, j, k, l, m, n)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i - 2, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n]
                                    if (j >= 2):
                                        new_index_set = (i, j - 2, k, l, m, n)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i, i] * mat[j - 2, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n]
                                    if (k >= 2):
                                        new_index_set = (i, j, k - 2, l, m, n)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k - 2, k] * mat[l, l] * mat[m, m] * mat[n, n]
                                    if (l >= 2):
                                        new_index_set = (i, j, k, l - 2, m, n)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l - 2, l] * mat[m, m] * mat[n, n]
                                    if (m >= 2):
                                        new_index_set = (i, j, k, l, m - 2, n)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m - 2, m] * mat[n, n]
                                    if (n >= 2):
                                        new_index_set = (i, j, k, l, m, n - 2)
                                        new_index = index_map[new_index_set]
                                        transformation[new_index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n - 2, n]

                                    # assuming max polynomial degree = 3, then cases would be 
                                    # (3, 0, 0, 0, 0, 0) or (2, 1, 0, 0, 0, 0) or (1, 1, 1, 0, 0, 0) or (0, 0, 0, 0, 0, 0)
                                    # i.e., at max 1 component has 2 parts
                                    # Note: degree 4 is much more complicated because it has 
                                    # (4, 0, 0, 0, 0, 0), (2, 2, 0, 0, 0, 0) and (3, 1, 0, 0)
                                    transformation[index, index] = mat[i, i] * mat[j, j] * mat[k, k] * mat[l, l] * mat[m, m] * mat[n, n]
                                    index += 1
