...
for i in range(parval):
    ...
    for ax, title, y_true, y_est in zip(axes.flat, titles, y_vals_true, y_vals_est):
        ...
        ax.plot(x_est[0, :], y_est, label='num intervals = '+str(meta_error_list[i][6].numsubintervals))
        ax.set_title(title)
        ax.grid(True)
...
for i in range(parval):
    f_est = f(np.array(meta_error_list[i][1].ordinary), x_est)
    ax.plot(f_est[1, :], f_est[2, :], f_est[3, :], label='num subintervals = '+str(meta_error_list[i][6].numsubintervals))
