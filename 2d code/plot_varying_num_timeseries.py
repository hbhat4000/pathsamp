...
# def f(theta, x):
#     evaluated_basis = np.zeros((x.shape[0], x.shape[1], prm.dof))
#     y = np.zeros((x.shape[0], x.shape[1]))
#     # print(x.shape, y.shape, evaluated_basis.shape)
#     # similar to the drift function in newbridge
#     for i in range(prm.dim):
#         evaluated_basis[i, :, :] = nb.hermite_basis(np.transpose(x))
#         y[:, i] = np.sum(np.dot(evaluated_basis[i, :, :], theta[:, i]))
#     return y
